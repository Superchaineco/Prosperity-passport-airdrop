...
import {Sunny} from "../src/Sunny.sol";
...
contract SuperchainERC20Deployer is Script {
        ...
        returns (address addr_, address ownerAddr_)
    {
        ...
        string memory name = "Sunny_MOCK";
        string memory symbol = "SUNNYLFG";
        uint256 decimals = 18;

        require(decimals <= type(uint8).max, "decimals exceeds uint8 range");
        bytes memory initCode = abi.encodePacked(
            type(Sunny).creationCode,
    ...
    function _implSalt() internal view returns (bytes32) {
        ...
        return keccak256(abi.encodePacked("Meaning of the life is42"));
    }
}
